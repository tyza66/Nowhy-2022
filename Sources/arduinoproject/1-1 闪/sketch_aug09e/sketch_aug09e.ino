...
void setup(void){
  ...
  wifiMulti.addAP("ChinaUnicom-THN6DT", "13050073610ld"); // 将需要连接的一系列WiFi ID和密码输入这里
  ...
  esp8266_server.on("/LEDoff", HTTP_POST, handleoffLED);// 设置处理LED控制请求的函数'handleLED'
...
void handleRoot() {       
  esp8266_server.send(200, "text/html", "<form action=\"/LED\" method=\"POST\"><input type=\"submit\" value=\"on LED\"></form><form action=\"/LEDoff\" method=\"POST\"><input type=\"submit\" value=\"off LED\"></form>");
}
